...
public class ShadedItemTagHandler : ITagHandler {
	public class ShadedItemSnippet(Item item) : TextSnippet {
		...
		public override bool UniqueDraw(bool justCheckingString, out Vector2 size, SpriteBatch spriteBatch, Vector2 position = default, Color color = default, float scale = 1f) {
			...
			if (Main.netMode != NetmodeID.Server && !Main.dedServ) {
				...
				if (Main.itemAnimations[item.type] != null) {
					Main.itemAnimations[item.type].GetFrame(value);
				} else {
					value.Frame();
				}
			}
			...
			if (!justCheckingString && color != Color.Black) {
				...
				ItemSlot.Draw(spriteBatch, ref item, 14, position - new Vector2(10f) * scale * num, color);
		...
		if (options != null) {
			...
			for (int i = 0; i < array.Length; i++) {
				...
				switch (array[i][0]) {
					...
					case 's':
					case 'x': {
						if (int.TryParse(array[i].AsSpan(1), out int result3)) {
							item.stack = Utils.Clamp(result3, 1, item.maxStack);
						}
						break;
					}
					case 'p': {
						if (int.TryParse(array[i].AsSpan(1), out int result2)) {
